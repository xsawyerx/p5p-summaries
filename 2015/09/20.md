...
More tickets opened by Dan Collins and Brian Carpenter, results of fuzzing
perl and uncovering interesting ways to crash perl.

### [Perl #126191](https://rt.perl.org/Ticket/Display.html?id=126191)

Brian Carpenter found a bug causing a null pointer dereference, leading
to a segfault.

### [Perl #126192](https://rt.perl.org/Ticket/Display.html?id=126192)

Brian Carpenter found another bug causing a null pointer dereference,
lading to a segfault as well.

### [Perl #126193](https://rt.perl.org/Ticket/Display.html?id=126193)

Brian Carpenter found an assertion failure.

### [Perl #126199](https://rt.perl.org/Ticket/Display.html?id=126199)

Dan Collins found a double free problem. Vincent Pit was able to provide
a summary of this bug in the form of:
...
Dan Collins provides a segfault caused by the following reduced regex:
...
Dan Collins provided a patch for compiling perl on AmigaOS.
...
### [Perl #126150](https://rt.perl.org/Ticket/Display.html?id=126150)
...
Ricardo Signes bumped a bug raised which caused a segfault and Shlomi
...
Bulk88 provided a patch for optimizing some stack manipulation.
...
Victor ADAM opened several regex-related tickets:
...
Karl Williamson asked for comments on having spaces when using `\p` in
regular expressions.
...
Nicolas R. suggested removing "dead code" from core (specifically
...
At the beginning of the month Jarkko Hietaniemi raised a personal annoyance
with the fact that:

    ref $foo eq 'ARRAY'

actually gets compiled into a string `eq` against an `ARRAY` constant,
which makes it not just inefficient, but also a possible problem with
blessed references - whether arrayref or `ARRAY` package name.
...
Bob Kleemann hit a snag when trying to dereference a variable referencing
...
Bulk88 asks whether `AUTOLOAD` should be called when some methods of a tied
hash do not exist.
...
Bulk88 wondered about possibly calling `scalar keys` on a tied hash instead
...
raised by Jozef Mojzis, Dan Collins writes that a side effect of fixing at
...
Father Chrysostomos adds a reasonable use-case for `stat(@_)` and suggests
...
wondered whether the `XS_RETURN` family of macros could introduce a segfault
...
Doug Bell asked what should happen when `XSRETURN` received a negative
...
Dave Mitchell sent another email with a list of temporary Git branches
...
[Encode](https://metacpan.org/pod/Encode) is out. Biggest change is also
